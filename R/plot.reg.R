#' @title plot the result of univariate regression
#'
...
plot.reg <- function(x,limits=c(NA,NA),sort="order",...) {
  ...
  stopifnot((is.na(limits[1])||is.numeric(limits[1]))&&(is.na(limits[2])||is.numeric(limits[2])))

  data<-x$dataframe
  names(data)[(NCOL(x$dataframe)-2):NCOL(x$dataframe)]<-c("center","low","high")

  if (is.na(limits[1])) limits[1]<-min(data$low,na.rm =TRUE)
  if (is.na(limits[2])) limits[2]<-max(data$high,na.rm =TRUE)
  ...
  data<-cbind(data,beyond1=ifelse(data$low<limits[1]-adj,TRUE,NA),beyond2=ifelse(data$high>limits[2]+adj,TRUE,NA))
  ...
  p <- ggplot(data, aes(term, center, ymin = low, ymax = high, color = term)) +
    ...
    theme(plot.title = element_text(size = 30),
          axis.text.x = element_text(hjust = 1, vjust = 0, size = 8),
          legend.position = "none") +
    coord_flip(ylim=limits)+
    geom_segment(aes(x=term,y=low*beyond2,xend=term,yend=limits[2]*beyond2+adj,group=term),arrow = arrow(type = "closed", length = unit(0.15, "inches")),na.rm=TRUE)+
    geom_segment(aes(x=term,y=high*beyond1,xend=term,yend=limits[1]*beyond1-adj,group=term),arrow = arrow(type = "closed", length = unit(0.15, "inches")),na.rm=TRUE)

  p<-switch (x$detail$call$model,
             lm=p+ylab("coefficients(95%CI)"),
             glm =p+ylab("OR(95%CI)"),
             coxph=p+ylab("HR(95%CI)")
  )
  return(p)
}
