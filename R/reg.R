...
#' @param x Column indices of the variables to be included in univariate analysis, the default columns are all the variables except dependent variable
...
reg <- function(data = NULL, x = NULL, y = NULL, factor = NULL, model = NULL, time = NULL,
    ...) {
    ...
    if (y %in% x) {
        warning(paste0("Column indice ", y, " in x and y, please check it out."), call. = FALSE)
        x = setdiff(seq_along(1:NCOL(data)), y)
    }
    if (is.null(factor)) {
        data <- data
    } else if (is.numeric(factor)) {
        data[, factor] <- lapply(data[, factor], factor)
    } else stop("factor indices should be a numeric vactor or NULL!", call. = FALSE)
    ...
    result_detail <- result_dataframe <- NULL
    ...
    for (i in x) {
        ...
        if (model == "lm") {
            ...
            fit <- survival::coxph(survival::Surv(data[, time], data[, y]) ~ data[, i],
                ...)
    ...
    result_dataframe$row.names <- sub("data[, i]", "", result_dataframe$row.names, fixed = TRUE)
    ...
    if (model == "lm") {
        names(result_dataframe) <- c("term", "estimate", "std.error", "statistic", "p.value",
            "coef", "coef.low", "coef.high")

    } else if (model == "glm") {
        names(result_dataframe) <- c("term", "estimate", "std.error", "statistic", "p.value",
            "OR", "OR.low", "OR.high")
    } else if (model == "coxph") {
        names(result_dataframe) <- c("term", "estimate", "HR", "std.error", "statistic",
            "p.value", "HR.low", "HR.high")
        result_dataframe <- result_dataframe[, c("term", "estimate", "std.error", "statistic",
            "p.value", "HR", "HR.low", "HR.high")]
    }
