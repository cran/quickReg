...
#' @param all_var A logical, whether to display all variables at specific p value cut-off, if \code{FALSE}, only display the first ten variables
...
display.reg <- function(x, cuts = c(0.001, 0.01, 0.05, 0.1, 1), all_var=TRUE,...) {
  ...
  cat("\n")
  counts <- sapply(cuts, function(p) list(`p-value` = sum(pvalue <= p),var=x$dataframe[x$datafram$p.value<p,]$term))
  ...
  for (i in 1:NROW(counts)) {
    if (all_var) {
      cat(paste0(row.names(counts)[i], ":  ", paste(unlist(counts[i,2]), sep=", ", collapse=", ")),collapse="\n\n\n")
    } else {
      cat(paste0(row.names(counts)[i], ":  ", paste(head(unlist(counts[i,2]),10), sep=", ", collapse=", ")),collapse="\n\n")
    }
  }
